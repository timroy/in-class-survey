...
# Importing Data
sv <- read_csv("Anonymous in-class survey (Responses) - Form responses 1.csv")
...
m1 <- lm(IPSCALE ~ LRSCALE + PRIV_WORK + PUB_WORK + NPR_PUB + PR_PUB + FLUENT + WOMAN + PID_STR, data = sv)
screenreg(m1)

# With individual fixed effects
m1_FE <- lm(IPSCALE ~ LRSCALE + PRIV_WORK + PUB_WORK + NPR_PUB + PR_PUB + FLUENT + WOMAN + PID_STR + factor(fixed_effect), data = sv)
screenreg(m1_FE)
...
ggplot() +
  ...
  geom_ribbon(data = plot_dat, aes(x = LRSCALE, ymax = upr, ymin = lwr), alpha = 0.5, color = "red") +
  geom_point(data = sv, aes(x = LRSCALE, y = IPSCALE))
